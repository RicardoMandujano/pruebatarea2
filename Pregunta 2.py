...
#crear una clase global llamada Gestor con las peliculas, cine, funcion
class GestorGlobal:
    def __init__(self):
        self.datos = []
    ...
    def listar_peliculas(self, cine):
        for dato in datos:
            if dato[0].nombre == cine:
                print("{} {}".format(dato[1].id, dato[1].id, dato[1].nombre))
    ...
    def guardar_entrada(self, pelicula_id, funcion_elegida, cantidad):
        pass
...
def main():
    ...
    gestorg = GestorGlobal()

    #def __init__(self, cine, pelicula, pelicula_id, funcion):
    gestorg.add(factorycine.obtener_cine("CinePlaneta"), 1, "Desaparecido", ["20:00", "23:00"])
